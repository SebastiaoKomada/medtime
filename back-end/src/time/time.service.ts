    ...
    while (this.isRunning) {
      ...
      for (const time of activeTimes) {
        if (!this.notifiedTimeIds.has(time.horId)) {
          ...
          if (profile) {
            console.log(`Nome: ${profile.perNome}, Horario ID: ${time.horId}, Horario: ${time.horario}`);
            const notificacao = { horId: time.horId, horMedId: time.horMedId, horario: time.horario };
            await this.notificationService.sendNotification(this.savedSubscription, 'Medtime', 'Hora de tomar seu medicamento!', 'CONFIRMAR');
